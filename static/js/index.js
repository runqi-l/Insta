...
function create_like(success_cb, error_cb) {
    ...
    //console will print the post pk
    console.log(post_pk);

    //ajax is used for single page rendering(on the same page, render a small part)
    //this ajax will POST request, visit url with data post_pk
    //json format
    $.ajax({
        type: "POST",
        url: '/like/',
        ...
        //if this ajax request succeed or failed handling
        //the data passed to success_cb is the return value from ajex in view.py
...
function like_update_view(data) {
    ...
    //find this post's likes
    var $likes = $post.find('.likes');
    //find the post's likes' count
...
// $ means go to html find all attributes with name submit-like
// success_cb is like_update_view
